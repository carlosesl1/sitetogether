...
export function FullTruckloadHero({
  sector,
  content,
  allowedAnchors,
  titleSize = "default",
}: FullTruckloadHeroProps) {
  const { image } = content;
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute inset-0 bg-gradient-to-b from-[#fffdf8] via-[#fffdf8]/92 sm:bg-gradient-to-r sm:from-[#fffdf8] sm:via-[#fffdf8]/90 sm:to-transparent",
          content.nextStep || (sector === "transporte-fracionado" && content.supportingText)
            ? "to-[#fffdf8]/85"
            : "to-[#fffdf8]/30",
        )}
      />
      ...
      <div className="container relative z-10 mx-auto flex min-h-[640px] items-center px-5 py-16 sm:min-h-[680px] sm:px-6 lg:min-h-[700px] lg:py-20">
        <div className="max-w-[860px]">
          ...
          {content.supportingText ? (
            <p className="mt-4 max-w-2xl text-base font-semibold leading-relaxed text-neutral-700 sm:text-lg">
              {content.supportingText}
            </p>
          ) : null}
          ...
          {content.nextStep ? (
            <p className="mt-5 max-w-xl text-sm font-medium leading-relaxed text-neutral-600">
              {content.nextStep}
            </p>
          ) : null}
        </div>
